...
static GLfloat lightx = 0;
static GLfloat lighty = 0;
...
void mouseMoveHandler(int x, int y)
{
  ...
  case 1: // Calculate the rotations
	  if (cam == 1) {
		  lightx = lightx + (x - startX);
		  lighty = lighty + (y - startY);
	  }
	  else {
...
void setUserView()
{
  ...
  if (cam == 1) {
	  glPushMatrix();
	  glLoadIdentity();
	  glRotated(lightx, 1.0, 0.0, 0.0);
	  glRotated(lighty, 0.0, 1.0, 0.0);
  }
}
