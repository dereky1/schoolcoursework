...
def separate(ll,p):
    true_ll, false_ll = None,None
    
    while ll != None and ll.value != None:
        if p(ll.value):
...
def is_min_heap(t):
    ...
    if t.left != None and t.value >= t.left.value: return False
    if t.right != None and t.value >= t.right.value: return False
...
class StringVar_WithHistory(StringVar):
    def __init__(self):
        self.str_history = [StringVar._default]
        StringVar.__init__(self)
    
    def set(self, value):
        if value != self.str_history[-1]:
            self.str_history.append(value)
            return self._tk.globalsetvar(self._name, value)
    
    def undo (self):
        if len(self.str_history) != 2:
            self.str_history.pop(-1)
            return self._tk.globalsetvar(self._name, self.str_history[-1])
